...
export default function Home() {
    ...
    useEffect(() => {
        axios.get("/news?publishState=2&_expand=category").then((res) => {
            ...
            const pieList = res.data.filter(item => item.author === username)
                ...
                <Col span={8}>
                    ...
                    >
                        <Meta
                            ...
                            title={username}
                            description={<div>
                                <b>{region ? region : "Global"}</b><span style={{ marginLeft: "20px" }}>{roleName}</span>
                            </div>}
                        />
                    </Card>
                </Col>
            </Row>
            <Drawer title="个人News Categories分析" placement="right" onClose={() => setVisible(false)} visible={visible} width="500px">
